...
class Util {
	...
	static final Pattern DUAL_LAST_NAME_PATTERN = ~ ".*(о|ій|ай|ич|ач|ик|ук|юк|як|ак|аш|яш|ар|яр|ун|ян|ин|ів|[сцтн]ь|сон|сен|ес|ез) "
...
//		log.debug("\t\t"  + posFlag + ", " + modifiers)
	...
	def print_stats(List<String> lines, int double_form_cnt) {
		...
		new File("stats", "dict_stats.txt").withWriter("utf-8") { stat_f ->
			...
			stat_f.printf("  словникових лем (без advp/bad/slang, без омонімів imperf/perf) %s\n", (cnt_std - double_form_cnt))
			stat_f.print("\nЧастоти за тегами:\n")
