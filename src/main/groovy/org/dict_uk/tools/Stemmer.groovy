...
//        println "ges: $cntGes"
    ...
    static String prefixes = "все|усе" \
        + "|авіа|авто|а[ву]діо|агро|аеро|аква|анти|багато|біо|важко|вело|взаємо|високо|відео|віце|внутрішньо|гідро|гіро|гіпер|загально|квазі|кібер|кіно" \
    ...
    final Pattern PREFIX_REMOVE = Pattern.compile(/^(авіа|авто|а[ву]діо|агро|аеро|аква|анти|багато|біо|важко|вело|взаємо|високо|відео|віце|внутрішньо|гідро|гіро|гіпер|загально|квазі|кібер|кіно/
        + /|мало|мега|мікро|міні|мото|мульти|нано|напів'?|псевдо|середньо|радіо|спец|теле|турбо|ультра|фото/
        + /|нео(?=[аеєиіїоуюя])|супер|двох?|тр(и|ьох)|чотир(и|ьох)|п'ят(и|ьох)|шести|семи|восьми|дев'яти|без|не)/)
...
//    static Pattern PREFIX = Pattern.compile(/^(по|над|про|за)/)
    static Map<String, Map<Pattern, String>> SUFFIXES = Map.of(
        " adj", [
            ...
            (Pattern.compile(/(поган|руськ|багат)ий$/)): '$1',
            ...
            (Pattern.compile(/(.{2,}?)([оеа])[ую]ва(ль)?ний$/)): '$1$2й',
...
//            (Pattern.compile(/(.{3,}?)[у]юваний$/)): '$1',
            ...
            (Pattern.compile(/(.{3,}?)((?<![аеєиіїоуюя])н?[ую]в)?атий$/)): '$1',
            
            (Pattern.compile(/(.{3,}?)(((?<![аеєиіїоуюя])н)?[ую]ва[нт]ий|іюваний|іюватий)$/)): '$1',
            (Pattern.compile(/(.{3,}?)(([іи]з|й|ь)?ований|([іи]з|(?<![аеєиіїоуюя])(л|і))?([ую]в)?альний|([іи]з|і)?[аея]?т[уо]рний)$/)): '$1',
...
//            (Pattern.compile(/(.{3,}?)((?<!н)[ио])цтво$/)): '$1$2к',
            ...
            (Pattern.compile(/(.{3,}?)(?<![аеєиіїоуюя])(((ен|')?я|а)т(оч)?ко|ечко|овиння|[вл]?[ея]ння)$/)): '$1',
            (Pattern.compile(/(.{3,}?)[уе](я|їст(ка)?|їстика|їстичність|їзм)$/)): '$1',
            (Pattern.compile(/(.{3,}?)([о])(їст(ка)?|їстика|їстичність|їзм)$/)): '$1$2й',
//            (Pattern.compile(/(.{3,}?)([ая])ка$/)): '$1',
            ...
            (Pattern.compile(/(.{3,}?)((?<![аеєиіїоуюя])н?[ую]в)?атість$/)): '$1',
            (Pattern.compile(/(.{3,}?)([уо]н)ня$/)): '$1$2',
            (Pattern.compile(/(.{2,}?)([оа])йованість$/)): '$1$2й',
            (Pattern.compile(/(.{3,}?)(((?<![аеєиіїоуюя])[ні])?[ую]ва([тн]ість|льність|ння)|([іи]з|і?й)?ованість|(а?[іїи]ст)?[іи]чність|[аи]ність|([іи]з|ер)?(іон)?[ую]вання)$/)): '$1',
            ...
            (Pattern.compile(/(.{3,}?)(і?[ую]вальни(к|ця)|ерка|ікат|(?<![аеєиіїоуюя])і?ат(ка)?|(?<![аеєиіїоуюя])ція|((ер)?[иі]з|і)?[ая]ція|(яц)?[еі]йник)$/)): '$1',
            (Pattern.compile(/(.{3,}?)([еі]йство|(ер)?ь?ство)$/)): '$1', // байкерство
            (Pattern.compile(/(.{3,}?)(стість|(яц)?[еі]йність|([іу]?ал)?(?<![аеєиіїоуюя])ь?[клнт]ість|ість|(ів)?(ни)?[цс]?тво)$/)): '$1', // :n:
            (Pattern.compile(/(.{3,}?)(([тджл])\2я)$/)): '$1',
            (Pattern.compile(/(.{3,}?)((і?[ую]в)?ач(ка)?|онька)$/)): '$1',
            ...
            (Pattern.compile(/(.{3,}?)((іон|і|а)?[іиї]ст(оч)?ка|(іон|і|а)?[іиї]ст(ика|ія)|(іон|і|а)?[иії]ст|(іон|і|а)?[иії]зм|унізм)$/)): '$1',
            (Pattern.compile(/(.{3,}?)ика$/)): '$1',
            (Pattern.compile(/(.{3,}?)([аоя])ч(ок|ка|атура)$/)): '$1', // (але тачка)
            (Pattern.compile(/(.{3,}?)(івець|іоніст(ка)?|ець|ієць|ятор|(([іи]з)?і?а)?тор|ей|ерик|ік(иня)?)$/)): '$1', // крад/ійка (але дівк/а)
