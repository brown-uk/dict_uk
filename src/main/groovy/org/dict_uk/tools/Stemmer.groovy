...
//        println "ges: $cntGes"
    ...
    static String prefixesBase = /авіа|авто|а[ву]діо|агро|аеро|аква|анти|багато|біло|біо|бого|важко|велико|вело|взаємо|високо|відео|віце|внутрішньо/ \
        + /|гідро|гіро|гіпер|етно|енерго|загально|квазі|кібер|кіно/ \
        + /|мало|мега|мікро|міні|мото|мульти|нано|напів'?|псевдо|само|середньо|радіо|спец|теле|турбо|ультра|фото/ \
...
//            (Pattern.compile(/(ор(юв)?)ане$/)): '$0',
            ...
            (Pattern.compile(/(.{3,}?)(?<![аеєиіїоуюя])ев(ість|е)$/)): '$1',
...
//            (Pattern.compile(/(.{3,}?)((?<!н)[ио])цтво$/)): '$1$2к',
            (Pattern.compile(/(.{3,}?)(иство|ивність|ивіст(ка)?|ивістика|ивізм|ивізація|ивізатор(ка)?|ивчик)$/)): '$1',
            (Pattern.compile(/(.{3,}?)(ьщина|атина)$/)): '$1',
