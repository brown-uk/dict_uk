...
//    static Pattern PREFIX = Pattern.compile(/^(по|над|про|за)/)
    static Map<String, Map<Pattern, String>> SUFFIXES = Map.of(
        " adj", [
            ...
            (Pattern.compile(/(.{2,}?)([оа])[ую]ва(ль)?ний$/)): '$1$2й',
            (Pattern.compile(/(.{3,}?)[ую]вальний$/)): '$1',
...
//                (Pattern.compile(/(нин)$/)) : 'а'
        ...
        " noun", [
            ...
            (Pattern.compile(/(.{2,}?)([лтч])\2я(чко)?$/)): '$1$2',
            (Pattern.compile(/(.{2,}?)єння$/)): '$1й',
            
            (Pattern.compile(/(.{3,}?)(?<![аеєиіїоуюя])люва(ння|(ль)?ність)$/)): '$1',
            (Pattern.compile(/(.{2,}?)([оа])[ую]ва(ння|ність)$/)): '$1$2й',
...
//            (Pattern.compile(/(.{3,}?)([ая])ка$/)): '$1',
            ...
            (Pattern.compile(/(.{3,}?)([ую]вальни(к|ця)|ерка|ікат|і?ат(ка)?|(?<![аеєиіїоуюя])ція|((ер)?[иі]з)?[ая]ція|[еі]йник|[еі]йство|(ер)?ь?ство)$/)): '$1', // дивакуватість, байкерство
            ...
            (Pattern.compile(/(.{3,}?)(стість|[еі]йність|(у?ал)?(?<![аеєиіїоуюя])ь?[клнт]ість|ість|([тджл])\3я|(ів)?(ни)?[цс]?тво)$/)): '$1', // :n:
            (Pattern.compile(/(.{3,}?)(ько|[чт]ко|е?н[еє]|стя|'я|[иі]ще|ь?це|іше)$/)): '$1', // :n:
