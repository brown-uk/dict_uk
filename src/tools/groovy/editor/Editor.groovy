...
def findInDict(word) {
	...
	def ending = word.replaceFirst(/^(авіа|авто|агро|аеро|анти|аудіо|багато|взаємо|відео|гео|гепато|геронто|геліо|гідро|гіпер|держ|еко|екстра|електро|за|кібер|кіно|мало|мега|мета|мікро|мото|нейро|не|пере|під|по|проти|про|псевдо|радіо|стерео|спорт|теле|фото|супер|термо)/, '')
